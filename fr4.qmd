...
pbase <- p1 +
  annotate("label", x=as.Date("2018-01-01"), y=20000, label="ARIMA futures", col='#888888')
p2 <- pbase +
  geom_line(
...
# Show less data
pbase$data <- train |> filter(year(Quarter) >= 2015)
p3 <- pbase +
  geom_line(
    data = filter(future, as.numeric(.rep) <= 5),
    aes(y = .sim, group = modrep, col=col),
...
# Add actuals
p7 <- p6 +
  geom_line(aes(y = Trips), data = aus_tourism |> filter(year(Quarter) >= 2015))
...
energy |>
  filter(Source %in% c('Total', 'Wind', 'Solar', 'Distillate')) |>
  mutate(
