...
\placefig{0.5}{2.5}{width=4cm}{monash_bw}
\placefig{14.6}{.2}{width=1.2cm}{iif_logo}
\begin{textblock}{10.5}(.5,0.3)
\fontsize{28}{40}\selectfont\bfseries\sffamily\textcolor[RGB]{204,89,0}{Forecast reconciliation}
\end{textblock}
\begin{textblock}{7.5}(.5,1.8)
\fontsize{13}{13}\selectfont\sffamily\textcolor[RGB]{80,70,60}{\bfseries Rob J Hyndman}
\end{textblock}
\begin{textblock}{8.5}(.4,3.5)
\begin{block}{}\fontsize{13}{14}\sf
\begin{enumerate}\itemsep=0cm\parskip=0cm
\item {Hierarchical time series and forecast reconciliation} {\footnotesize (Nov 6/7)}
\item Perspectives on forecast reconciliation {\footnotesize (Nov 9/10)}
\item \textbf{Probabilistic forecast reconciliation} {\footnotesize (Nov 13/14)}
\item Temporal and cross-temporal forecast reconciliation {\footnotesize (Nov 16/17)}
\end{enumerate}
\end{block}
\end{textblock}
...
\begin{textblock}{9.5}(5.3,1.1)\fontsize{13}{13}\sf
\begin{itemize}\tightlist
\item We want to forecast many time series that are related due to aggregation (or linear) constraints.
\item Forecast reconciliation involves forecasting all series, and then adjusting the forecasts to impose the constraints.
\item The optimal ($L_2$) solution is known as ``Minimum Trace'' reconciliation.
\item It is a linear projection, and we can derive many properties using the theory of projections.
\item There are many extensions to this basic idea, to get reconciled point forecasts.
\end{itemize}
\end{textblock}
...
\begin{alertblock}{}
\centerline{$\tilde{\bm{y}} = \bm{S}\bm{G}\hat{\bm{y}} = \bm{S}\tilde{\bm{b}}$}
...
\begin{block}{}
Suppose that $\left(\hat{\bm{y}}^{[1]},\ldots,\hat{\bm{y}}^{[L]}\right)$ is a sample drawn from an incoherent probability distribution. Then $\left(\tilde{\bm{y}}^{[1]},\ldots,\tilde{\bm{y}}^{[L]}\right)$ where $\tilde{\bm{y}}^{[\ell]}:=\psi(\hat{\bm{y}}^{[\ell]})$ for $\ell=1,\ldots,L$, is a sample drawn from the reconciled probability distribution.
\end{block}\vspace*{-0.4cm}

* Simulate future sample paths for each series, by simulating from each model using a multivariate bootstrap of the residuals (to preserve \rlap{cross-correlations).}
...
# Example: Emergency Services Demand
...
* Scaled CRPS: relative to CRPS from Naïve forecasts.
...
\begin{block}{Continuous Rank Probability Score (univariate forecasts)}
Forecast distribution $F_t$ and observation $y_t$.
\centerline{$\text{CRPS}(F_t,y_t)~  = \int_{0}^1 S_{p,t}(p,y_t) dp ~  = ~\text{E}_F|Y-y_t|-\frac{1}{2}\text{E}_F|Y-Y^*|$}\vspace*{-0.37cm}
\begin{itemize}\tightlist
\item $Y$ and $Y^*$ are iid draws from $F_t$.
\item Optimal when $F_t$ is truth (i.e., it is a proper score)
\end{itemize}
\end{block}
\pause
\begin{block}{Energy score (multivariate forecasts)}
\begin{itemize}
\item $\text{ES}(F_t,\bm{y}_t) = \E_{F} ||{\bm{Y}}-\bm{y}_t|| -\frac{1}{2}\E_{F}||\bm{Y}-\bm{Y}^*||$
\end{itemize}
\end{block}
\pause
\begin{block}{Log score (multivariate forecasts)}
\begin{itemize}
\item $\text{LS}(F_t, \bm{y}_t) = -\log f(\bm{y}_t)$
\end{itemize}
\end{block}
...
\alert{Comparing coherent vs incoherent}

* Log score is not proper
* Energy score is proper

\pause\alert{Comparing coherent vs coherent}

* Both Energy score and Log score are proper
* Ordering preserved if compared using bottom-level only
* Full hierarchy should be used.
* Rankings may change otherwise.
...
\begin{block}{Daily time series from \url{opennem.org.au}}\fontsize{10}{11}\sf\hspace*{-0.2cm}
\begin{tikzpicture}
...
\tikzstyle{level 3}=[sibling distance=11mm,font=\footnotesize,set style={{every node}+=[fill=yellow]}]
...
\nocite{smartmeterhts,coherentprob,fem,JeoEtAl2019}
