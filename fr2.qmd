...
\begin{tikzpicture}
  ...
  % Draw vectors
  \draw[->,thick,blue] (O) -- (A) node[midway,anchor=south] {$\vec{a}$};
  \draw[->,thick,blue] (O) -- (B) node[midway,anchor=north west] {$\vec{b}$};

  % Draw projection
  \coordinate (P) at ($(A)!(B)!(O)$);
  \draw[->,thick,red,dashed] (B) -- (P) node[midway,anchor=south] {$\vec{p}$};
  \draw[dashed] (P) -- (A);

  % Draw angle
  \draw[->,thick,gray] (0.7,0) arc (0:26.565:0.7) node[midway,anchor=west] {$\theta$};
\end{tikzpicture}
